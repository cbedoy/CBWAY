...
public abstract class CBMaps implements IMapDelegate{
    ...
    protected void storageRequest(String urlRequest,String info,String status,
        ...
        Date date = new Date();
        numRequest+=1;
        CBMaps.stockRequest=(String[][])(this.resizeArray(CBMaps.stockRequest,numRequest));
        if(CBMaps.stockRequest[numRequest-1]==null){
                CBMaps.stockRequest[numRequest-1]=new String[6];
            }
        CBMaps.stockRequest[numRequest-1][0]=String.valueOf(numRequest);
        CBMaps.stockRequest[numRequest-1][1]=date.toString();
        CBMaps.stockRequest[numRequest-1][2]=status;
        CBMaps.stockRequest[numRequest-1][3]=urlRequest;
        CBMaps.stockRequest[numRequest-1][4]=info;
        if(exception==null){
            CBMaps.stockRequest[numRequest-1][5]="No exception";
        }else{
            CBMaps.stockRequest[numRequest-1][5]=exception.toString();
        }
        
    }
    
    protected String getSelectPropertiesRequest(){
        return "&region=" + CBMaps.region + "&language=" + CBMaps.language +
                "&sensor=" + CBMaps.sensor;
    }
    ...
    */
     ...
     * @see cbedoy.cbway.lib.CBMaps#setKey(String)
     * @see cbedoy.cbway.lib.CBMaps#getKey()
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#setConnectTimeout(int)
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#getConnectTimeout()
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#setRegion(String)
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#getRegion()
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#setLanguage(String) ()
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#getLanguage()
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#setSensor(Boolean)
     */
     ...
     * @see cbedoy.cbway.lib.CBMaps#getSensor()
     */
    ...
    /**
     * Obtiene la clave actual de desarrollador de API Google Maps (sólo necesario para CBPlaces)
     * @return obtiene string con clave actual
     * @see cbedoy.cbway.lib.CBMaps#setKey(String)
     * @see cbedoy.cbway.lib.CBMaps#APIkeyCheck(String)
     */
    ...
    /**
     * Establece clave de desarrollador API Google Maps (sólo necesario para CBPlaces)
     ...
     * [0][3]=""http://maps.google.com/maps/api/geocode/xml?address=Madrid&region=es&language=es&sensor=false"";<br/>[0][4]="CBGeocoding request";<br/>[0][5]="No exception";<br/>
     * Tipos de status:<br/>
     ...
     * [3]="http://maps.google.com/maps/api/geocode/xml?address=Madrid&region=es&language=es&sensor=false";<br/>[4]="CBGeocoding request";<br/>[5]="No exception";<br/>
     * Tipos de status:<br/>
     * OK: indica que la solicitud del API se realizó correctamente.<br/>
     * INVALID_REQUEST: indica que la solicitud del API se formó de forma incorrecta.<br/>
     * OVER_QUERY_LIMIT: indica que el solicitante ha superado los límites.<br/>
     ...
     * @see cbedoy.cbway.lib.CBMaps#getStockRequest()
     */
     ...
     * @return retorna string con información de la última petición realizada (por ejemplo, "CBGeocoding request")
     * @see cbedoy.cbway.lib.CBMaps#getStockRequest()
     */
